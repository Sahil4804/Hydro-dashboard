...
import {
  ...
  Radar, Legend, ReferenceLine, Cell,
} from "recharts";
...
export default function StreamModelsPage() {
  const { data: cvSummary, isLoading: l1, error: e1 } = useApi<any[]>("/api/streamflow/models/cv-summary");
  const { data: cvFolds, isLoading: l2, error: e2 } = useApi<any[]>("/api/streamflow/models/cv-folds");
  const { data: testMetrics, isLoading: l3, error: e3 } = useApi<any[]>("/api/streamflow/models/test-metrics");
  const { data: testPreds, isLoading: l4, error: e4 } = useApi<any[]>("/api/streamflow/models/test-predictions");
  const { data: featureImp, isLoading: l5, error: e5 } = useApi<any[]>("/api/streamflow/models/feature-importance");
  const { data: residuals, isLoading: l6, error: e6 } = useApi<any[]>("/api/streamflow/models/residuals");
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Streamflow - ML Model Performance</h1>
        <p className="text-sm text-muted-foreground">Comparing Linear Regression, Random Forest, and SVR</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <TableBody>
              {(cvSummary || []).map((row: any) => (
                <TableRow key={row.model}>
                  ...
                  <TableCell>{row.rmse?.toFixed(4)}</TableCell>
        ...
        <ChartCard title="Model Comparison Radar" description="Normalized metrics">
      ...
      <ChartCard title="Observed vs Predicted Streamflow (2022-2024)">
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={testPreds || []}>
            ...
            <Line dataKey="streamflow_m3s" name="Observed" stroke={COLORS.historical} strokeWidth={2.5} dot={{ r: 3 }} />
            <Line dataKey="baseline_pred" name="Baseline" stroke={MODEL_COLORS.Baseline} strokeDasharray="5 5" strokeWidth={1.5} dot={false} />
            <Line dataKey="svr_pred" name="SVR" stroke={MODEL_COLORS.SVR} strokeWidth={2} dot={{ r: 2 }} />
            <Line dataKey="random_forest_pred" name="Random Forest" stroke={MODEL_COLORS["Random Forest"]} strokeWidth={1.5} dot={false} />
        ...
        <ChartCard title="Feature Importance (Random Forest)">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={featureImp || []} layout="vertical">
              ...
              <Tooltip contentStyle={{ fontSize: 12 }} />
              <Bar dataKey="importance" fill={COLORS.streamflow} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>

        <ChartCard title="Test Metrics Comparison">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Model</TableHead>
      ...
      {/* Residuals */}
      <ChartCard title="Residuals Over Time (SVR)" description="Observed - Predicted">
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={residuals || []}>
            ...
            <Bar dataKey="svr_pred_residual" name="Residual (m3/s)">
              {(residuals || []).map((entry: any, i: number) => (
                <Cell key={i} fill={(entry.svr_pred_residual || 0) >= 0 ? COLORS.positive : COLORS.negative} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
